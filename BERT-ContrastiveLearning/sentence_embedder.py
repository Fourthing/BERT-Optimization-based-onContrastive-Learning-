...
class SentenceEmbedder:
    def __init__(self, model_name):
        self.tokenizer = BertTokenizer.from_pretrained(model_name)
        self.model = BertModel.from_pretrained(model_name)  # 使用 BertModel
    ...
    def get_sentence_embeddings(self, sentences):
        ...
        with torch.no_grad():
            for sentence1, sentence2 in sentences:
                ...
                # 使用 last_hidden_state 而非 SequenceClassifierOutput
                sentence_embedding1 = outputs1.last_hidden_state[:, 0, :]  # 提取 [CLS] token 的嵌入
                sentence_embedding2 = outputs2.last_hidden_state[:, 0, :]  # 提取 [CLS] token 的嵌入
